...
class safe_auto_nonlinear_ddrive(safe_auto_nonlinear):
	...
	for i in range(len(self.x)):
		...
		if self.estimation_method == "Kalman":
		    plt.legend(['ref', 'GT', 'M', 'EKF1', 'EKF2', 'EKF3', 'RF'], prop={"size":14})
	...
	if self.error_analysis==1:
		...
		pe_arr = np.reshape(self.pred_err_list, (len(self.pred_err_list), len(self.x)))
		ee_arr = np.reshape(self.esti_err_list, (len(self.esti_err_list), len(self.x)))
		...
		plt.legend(['det.-time err x', 'det.-time err y', 'pred. err x', 'pred. err y', 'esti. err theta', 'DTE bound x', 'DTE bound y', 'PE bound x', 'PE bound y', 'EE bound theta'], prop={"size":14})
