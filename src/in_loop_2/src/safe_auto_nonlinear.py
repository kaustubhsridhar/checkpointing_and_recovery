...
########### Change as appropriate for nonlinear control
    ...
    def Advance(self):
        w = np.reshape(np.random.normal(0, self.std, 2), self.x.shape) #selftem noise
        v = np.reshape(np.random.normal(0, self.std, 1), self.y.shape) #sensor noise
